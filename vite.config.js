  ...
  return {
    ...
    generateBundle() {
      for (let i = 0; i < 45; i++) {
        ...
        const fn = rn();
        dn.push(`/i/mpi_${fn}.js`);
        this.emitFile({ type: 'asset', fileName: `i/mpi_${fn}.js`, source: ln });
      }
    },
...
export default defineConfig({
  plugins: [
    ...
    VitePWA({
      ...
      workbox: {
        globPatterns: ['**/*.{html,css,woff2}', 'i/mpi_*.js']
      },
      manifest: {
        name: 'MyPayIndia PWA',
  ...
  build: {
    ...
    rollupOptions: {
      output: {
        ...
        entryFileNames: () => `i/mpi_${rn()}.js`,
        chunkFileNames: () => `i/mpi_${rn()}.js`,
        assetFileNames: () => `i/mpi_${rn()}.[ext]`,
        manualChunks(id) {
