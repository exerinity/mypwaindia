...
export function CliTerminal({ variant = 'page', fullscreen = false, active: visible = true, onExit }: CliTerminalProps) {
  ...
  const cmdHistory = useSyncExternalStore(subscribe, getCmdHistory);
  ...
  async function runCmd(raw: string) {
    ...
    switch (c) {
      ...
      case 'fly': {
        ...
        if (onCliPage) {
          if (!settings.cliDrawer) {
            ...
            if (answer !== 'y' && answer !== 'yes') { push(L.warn('cancelled')); return; }
            updateSettings({ cliDrawer: true });
          }
          push(L.info('OK'));
          const home = location.pathname.startsWith(settings.homePage) ? '/dash' : settings.homePage;
          requestDrawerOpen();
          navigate(home);
        } else {
          push(L.info('OK'));
      ...
      case 'drawer':
      case 'dr': {
        const next = !settings.cliDrawer;
        updateSettings({ cliDrawer: next });
        push(next ? L.ok('Drawer ON') : L.warn('Drawer OFF'));
        return;
      }
